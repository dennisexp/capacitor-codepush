...
This plugin provides client-side integration for the [CodePush service](https://github.com/dennisexp/code-push-server/), allowing you to easily add a dynamic update experience to your Capacitor app(s).
...
npm i https://github.com/dennisexp/capacitor-codepush -D
npx cap sync
```
...
1. Add your deployment & code signing (optional) keys to the `capacitor.config.json` file, making sure to include the right key:
    ...
    "Plugins": {
        ...
        "CodePush": {
          ...
          "SERVER_URL": "https://your-own-codepush-server-url"
        }
    }
    ```

    As a reminder, the *deployment* keys are generated for you when you created your CodePush app via the CLI. If you need to retrieve them, you can simply run `code-push deployment list --app <ownerName>/<appName> --displayKeys`, and grab the key for the specific deployment you want to use (e.g. `Staging`, `Production`). App IDs can be retrieved by running `appcenter apps list`.
...
2. To ensure that your app can access the CodePush server on [CSP](https://developer.mozilla.org/en-US/docs/Web/Security/CSP/Introducing_Content_Security_Policy)-compliant platforms, add `https://your-own-codepush-server-url` to the `Content-Security-Policy` `meta` tag in your `index.html` file:

    ```xml
    <meta http-equiv="Content-Security-Policy" content="default-src https://your-own-codepush-server-url 'self' data: gap: https://ssl.gstatic.com 'unsafe-eval'; style-src 'self' 'unsafe-inline'; media-src *" />
    ```
...
*NOTE: Before you can start releasing updates, please log into App Center by running the `code-push login` command*

In it's the most basic form, this command only requires one parameter: your owner name + "/" + app name.

```shell
# Generic
code-push release <ownerName>/<appName> -c public/

# iOS
code-push release <ownerName>/MyApp-ios -c ios/App/App/public/

# Android
code-push release <ownerName>/MyApp-Android -c android/app/src/main/assets/public/
```
...
code-push release <ownerName>/MyApp-ios -c ios/App/App/public/ -m --description "Modified the header color"

# Release a dev Android build to just 1/4 of your end users
code-push release  <ownerName>/MyApp-android -c android/app/src/main/assets/public/ --rollout 25

# Release an update that targets users running any 1.1.* binary, as opposed to
# limiting the update to exact version name in the config.xml file
code-push release  <ownerName>/MyApp-android -c android/app/src/main/assets/public/ --target-binary-version "~1.1.0"

# Release an update now but mark it as disabled
# so that no users can download it yet
code-push release  <ownerName>/MyApp-ios -c ios/App/App/public/ -x

# Release an update signed by private key (public key should be configured for application)
code-push release  <ownerName>/MyApp-android -c android/app/src/main/assets/public/ --privateKeyPath ~/rsa/private_key.pem
```
...
*NOTE: for Cordova, we used to use `appcenter codepush release-cordova`, but with Capacitor we must use `code-push release`.*
